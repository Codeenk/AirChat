...
class _GroupInfoScreenState extends ConsumerState<GroupInfoScreen> {
  ...
  Future<void> _load() async {
    final g =
        await ref.read(groupDaoProvider).getGroupById(_group.id) ?? _group;
    _group = g;
        ...
        builder: (ctx, setS) => AlertDialog(
          ...
          content: SizedBox(
            ...
            child: ListView.builder(
              itemCount: notIn.length,
              itemBuilder: (_, i) {
                final c = notIn[i];
                return CheckboxListTile(
                  value: picked.contains(c.uid),
                  onChanged: (v) =>
                      setS(() => v! ? picked.add(c.uid) : picked.remove(c.uid)),
                  title: Text(
                    c.username,
  ...
  Widget build(BuildContext context) {
    final date = DateFormat('MMM d, yyyy')
        .format(DateTime.fromMillisecondsSinceEpoch(_group.createdAt));
    return Scaffold(
      ...
      body: _loading
          ...
          : ListView(
              children: [
                ...
                Center(
                  child: Text(
                    'Created $date • ${_group.memberUids.length} members',
                    style: const TextStyle(
                ...
                ListTile(
                  ...
                  title: const Text(
                    'Add member',
                    style: TextStyle(color: AirColors.accent),
                  ),
                  onTap: _addMembers,
                ),
                const Divider(color: AirColors.divider, height: 1),
                  ...
                  child: SizedBox(
                    ...
                    child: OutlinedButton.icon(
                      ...
                      style: OutlinedButton.styleFrom(
                        side: const BorderSide(color: AirColors.error),
